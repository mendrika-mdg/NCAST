...
from timefeat import Core2MapModel
...
OUTPUT_BASE = f"/work/scratch-nopw2/mendrika/OB/ncast/evaluation/nowcasts/notimefeat/t{LEAD_TIME}"
...
# load model
MODEL_ROOT = f"/gws/nopw/j04/wiser_ewsa/mrakotomanga/OB/ncast/checkpoints/timefeat/t{LEAD_TIME}/seed1998"
...
print(f"Loaded NCAST time-feature ablation model: {CKPT_PATH}")
...
# scaler
COLS_TO_SCALE = range(0, 8)                     # do not scale the mask
...
for year, month, day, hour, minute in tqdm(input_files, desc="Predicting"):
    try:
        ...
        # remove temporal features: month_sin, month_cos, tod_sin, tod_cos
        X = X[:, :, 4:]
...
print("All NCAST time-feature ablation nowcasts completed on CPU.")