...
for file_path in tqdm(all_files, desc=f"Streaming ROC for t+{lead_time}"):
    ...
    gt = nd.maximum_filter(gt, size=L_pixels).astype(np.float32)
    pers = nd.maximum_filter(pers, size=L_pixels).astype(np.float32)
...
np.save(os.path.join(output_dir, f"roc_thresholds_t{lead_time}_smooth{L_pixels}.npy"), thresholds)

np.save(os.path.join(output_dir, f"roc_fpr_model_t{lead_time}_smooth{L_pixels}.npy"), FPR_model)
np.save(os.path.join(output_dir, f"roc_tpr_model_t{lead_time}_smooth{L_pixels}.npy"), TPR_model)

np.save(os.path.join(output_dir, f"roc_fpr_persistence_t{lead_time}_smooth{L_pixels}.npy"), FPR_pers)
np.save(os.path.join(output_dir, f"roc_tpr_persistence_t{lead_time}_smooth{L_pixels}.npy"), TPR_pers)

np.save(os.path.join(output_dir, f"roc_fpr_nflics_t{lead_time}_smooth{L_pixels}.npy"), FPR_nflics)
np.save(os.path.join(output_dir, f"roc_tpr_nflics_t{lead_time}_smooth{L_pixels}.npy"), TPR_nflics)

np.save(os.path.join(output_dir, f"auc_model_t{lead_time}_smooth{L_pixels}.npy"), auc_model)
np.save(os.path.join(output_dir, f"auc_persistence_t{lead_time}_smooth{L_pixels}.npy"), auc_pers)
np.save(os.path.join(output_dir, f"auc_nflics_t{lead_time}_smooth{L_pixels}.npy"), auc_nflics)
